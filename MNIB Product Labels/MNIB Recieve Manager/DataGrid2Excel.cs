...
namespace DataGrid2Excel
{
...

    {
        ...
        /// </summary>
        public void GenerateReport(DataTable grd)
        {
            try
            {
                dataToPrint = grd;
        ...
        private void WriteData(object[] header)
        {
            try
            {
                ...
                for (int j = 0; j < dataToPrint.Rows.Count - 1; j++)
                {
                    var item = dataToPrint.Rows[j];
                    if (item == null) continue;
                    for (int i = 0; i < header.Length; i++)
                    {
                        var y = header[i];
                        var val = (item as DataRow)[y.ToString()];
        ...
        private object[] CreateHeader()
        {
            ...
            {
                objHeaders.Add(dataToPrint.Columns[n].ColumnName);
            }
