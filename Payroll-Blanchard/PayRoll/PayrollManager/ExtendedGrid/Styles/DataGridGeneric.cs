...
namespace ExtendedGrid.Styles
{
    public partial  class DataGridGeneric
    {
                ...
                if (mainGrid != null)
                {
                    ...
                    if (mainGrid.AutoFilterHelper.CurrentColumName == CurrentColumn)
                    {
                        ...
                        foreach (var checkedListItem in currentValues)
                        {
                            ...
                            if (previousValues.Count(c => c.Name == item.Name && c.IsSelectAll != "(Select All)") == 0 && checkedListItem.IsSelectAll != "(Select All)")
                            {
            ...
            
            {
                ...
                case "Average":
                    extendedDataGrrid.RowSummariesTable.Rows[ExtendedDataGrid.Average][currentColumnName] =
                        Convert.ToDouble(itemSourceDataTable.Compute("Avg(" + currentColumnName + ")", ""));
                    break;
                case "Min":
                ...
                case "Largest":
                    ...
                    if (lengthOfCells1.Count > 0)
                    {
                        ...
                        extendedDataGrrid.RowSummariesTable.Rows[ExtendedDataGrid.Lasrgest][currentColumnName] = maxValue;
                    }

                    break;
            }
            ((ExtendedDataGrid)CurrentGrid).FireotiFyForRowSummaries();
        ...
        private void SigmaUnChecked(object sender, RoutedEventArgs e)
        {
            ...
            switch (currentComputation)
            {
                ...
                case "Smallest":
                    ...
                    break;
                case "Largest":
                    extendedDataGrrid.RowSummariesTable.Rows[ExtendedDataGrid.Lasrgest][currentColumnName] = "";
                    break;
            }

            foreach(DataRow row in extendedDataGrrid.RowSummariesTable.Rows)
            {
                if(!string.IsNullOrEmpty(Convert.ToString(row[currentColumnName])))
                {
        ...
        private void ClearSummaries(object sender, RoutedEventArgs e)
        {
            ...
            {
                var itemSource = mainGrid.AutoFilterHelper.CurrentSigmaListBox.ItemsSource as List<CheckedListItem>;
                if (itemSource != null)
                {
                ...
                if(summariesGrid!=null)
                {
                    ...
                    if (scrollViewer != null)
                    {
                        ...
                        if (border != null)
                        {
                            ...
                            if (scroll != null) scroll.ScrollToHorizontalOffset(e.NewValue);
                        }
                    }
                }
            }
