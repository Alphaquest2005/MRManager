...
namespace LinqLib.Sequence
{
  ...
  public static class Subset
  {
    ...
    public static IEnumerable<TSource> TakePattern<TSource>(this IEnumerable<TSource> source, int initialSkip, int take, int skip)
    {
      Helper.InvalidateNullParam(source, "source");
      if (take < 1)
        throw new ArgumentException(Resources.excptionInvalidTakeCount, "take");
      if (skip < 1)
        throw new ArgumentException(Resources.excptionInvalidSkipCount, "skip");
    ...
    {
      foreach (TSource current in TakeSelfAndBefore(source, item, before))
        yield return current;
      foreach (TSource current in TakeAfter(source, item, after))
        yield return current;
    }
    ...
    {
      foreach (TSource current in TakeSelfAndBefore(source, item, before, comparer))
        yield return current;
      foreach (TSource current in TakeAfter(source, item, after, comparer))
        yield return current;
    }
    ...
    public static IEnumerable<TSource> TakeTopPercent<TSource>(this IEnumerable<TSource> source, double percent) // 1 == 100%
    {
      ...
      int count = (int)((double)source.Count() * percent);
      return source.TakeTop(count);
    }
    ...
    public static IEnumerable<TSource> TakeTopPercent<TSource>(this IEnumerable<TSource> source, double percent, Func<TSource, bool> predicate) // 1 == 100%
    {
      ...
      int count = (int)((double)source.Count() * percent);
      return source.TakeTop(count, predicate);
    }
