...
namespace Core.Common.UI.DataVirtualization
{
    ...
    public partial class VirtualList<T> : IDisposable, IVirtualList, IList<VirtualListItem<T>>, INotifyPropertyChanged, INotifyCollectionChanged where T : IEntity
    {
        ...
        public VirtualList(IVirtualListLoader<T> loader, int pageSize, SynchronizationContext synchronizationContext)
        {
            ...
            EventMessageBus.Current.GetEvent<EntityRangeLoaded<T>>(new MessageSource(this.ToString()))
                .Subscribe(x => handleRangeLoaded(x.Entities,x.StartIndex, x.OverAllCount));
        ...
        private void handleRangeLoaded(IList<T> entities,int startIndex,  int overAllCount)
        {
        ...
        {
           EventMessageBus.Current.Publish(new LoadEntityRange<T>(startIndex, count, SortDescriptions,new MessageSource(this.ToString())),new MessageSource(this.ToString()));
        }
