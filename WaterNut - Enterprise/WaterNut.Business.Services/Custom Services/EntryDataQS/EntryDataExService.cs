...
namespace EntryDataQS.Business.Services
{
    ...
    public partial class EntryDataExService
    {
        ...
        {
            await WaterNut.DataSpace.BaseDataModel.Instance.RemoveEntryData(po).ConfigureAwait(false);
        }

        public async Task RemoveSelectedEntryData(IEnumerable<string> selectedEntryDataEx)
        {
            await
                WaterNut.DataSpace.EntryDataModel.Instance.RemoveSelectedEntryData(selectedEntryDataEx)
