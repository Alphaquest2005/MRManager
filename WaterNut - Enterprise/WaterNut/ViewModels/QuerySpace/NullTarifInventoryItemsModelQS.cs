    ...
    public partial class InventoryItemsModel : InventoryItemsExViewModel_AutoGen 
    {
        ...
        public override void FilterData()
        {
            ...
            if (ViewPOItems == true)
            {
                ...
                {
                    navExp.Append(string.Format("&& EntryDataId == \"{0}\"", CurrentEntryData.InvoiceNo));
            ...
            if (ViewDocSetItems == true)
            {
                ...
                {
                    navExp.Append(string.Format("&& CNumber == \"{0}\"", CoreEntities.ViewModels.BaseViewModel.Instance.CurrentAsycudaDocument.CNumber));
                }

                if (CoreEntities.ViewModels.BaseViewModel.Instance.CurrentAsycudaDocumentSetEx != null)
                {
                   navExp.Append(string.Format("&& AsycudaDocumentSetId == \"{0}\"", CoreEntities.ViewModels.BaseViewModel.Instance.CurrentAsycudaDocumentSetEx.AsycudaDocumentSetId));
                }
            }
        ...
        {
            StatusModel.Timer("Mapping Inventory to Asycuda");
            await InventoryItemsExRepository.Instance.MapInventoryToAsycuda().ConfigureAwait(false);
            StatusModel.StopStatusUpdate();
